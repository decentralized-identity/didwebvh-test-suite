...
fn all_witness_secrets(keys: &HashMap<String, KeyInfo>) -> HashMap<String, Secret> {
    ...
    for k in keys.values() {
        out.insert(k.pub_multibase.clone(), k.secret.clone());
...
async fn main() {
    ...
    for scenario in &scenarios {
        ...
        // Happy-path generator should not process negative scenarios.
        let script_is_negative = std::fs::read_to_string(&script_path)
            .ok()
            .and_then(|txt| serde_yaml::from_str::<Script>(&txt).ok())
            .map(|s| s.negative)
            .unwrap_or(false);
        if script_is_negative {
            continue;
        }
